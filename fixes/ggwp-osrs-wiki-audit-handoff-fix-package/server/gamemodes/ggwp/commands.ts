...
import { formatGgwpStaffChatPrefix, requireStaff } from "./auth";
...
export function registerGgwpCommands(registry: IScriptRegistry, services: ScriptServices): void {
    ...
    register(registry, ["yell"], ({ player, args }) => {
        ...
        const staffChatPrefix = formatGgwpStaffChatPrefix(player);
        const displayName = staffChatPrefix ? `${staffChatPrefix} ${playerName}` : playerName;
        broadcast(services, `[Yell] ${displayName}: ${message}`);
    });

# Quest + Dialogue Complete Final Mega Fix

This package is the final combined quest/dialogue fix for the OSRS rev 238 server project.
...
This package does not copy official OSRS dialogue transcripts and does not attempt to be a near-verbatim rewrite of Jagex dialogue.

The goal is:

- 100% quest-reference registration and state/routing coverage against the local 209 quest/miniquest reference.
- Playable English quest dialogue that preserves quest intent, state transitions, starter/helper/turn-in roles and rewards.
- Copyright-safe, original fallback dialogue for non-quest NPCs that do not have bespoke scripts.

If exact official dialogue is required, it must be added from a licensed/permitted source. Do not scrape and paste official transcript text into this package.
...
- Adds a finished non-quest `Talk-to` fallback so NPCs without quest/bespoke handlers no longer show placeholder text.
...
See `FINAL_DIALOGUE_COVERAGE_REPORT.md` for the broader NPC/dialogue coverage notes.
